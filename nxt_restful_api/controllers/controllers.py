...
class NxtRestfulApi(http.Controller):
    ...
    def get_token(self,serv='http://d10.appnxt.com', login=None, password='admin', db='d10', sid=None, success=True, message='',**kw):
        ...
        if not uid:
            ...
            return json_response(rp)
        e = base64.urlsafe_b64encode(','.join([serv,db,login,str(uid),str(int(time.time()))]).encode())
        rp = {'token': e.decode().replace('=',''),'success':success,'message':message}
        return json_response(rp)
                ...
                for f in r.keys():
                    if model_fields[f]['type'] == 'many2one':
                        if r[f]:
                            ...
                            r[f] = ''
                    if model_fields[f]['type'] == 'binary' and r[f]:
                        r[f] = r[f].decode()
